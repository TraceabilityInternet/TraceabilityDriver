    ...
    public static class GCPLookUp
    {
        ...
        public static string DetermineCompanyPrefix(string gtinOrgln)
        {
            try
            {
                ...
                {
                    throw new TEException("gtinOrgln has an invalid length, must be eiterh 13 or 14 digits long.");
                }

                if (!gtinOrgln.IsOnlyDigits())
                {
                    throw new TEException("glnStr is not only digits. This is not a proper GS1 GLN-13.");
                }
                ...
                using (LimitedPoolItem<HttpClient> limitedItem = _httpClientPool.Get())
                {
                    ...
                    {
                        TELogger.Log(0, "Failed to query the GS1 Data Hub for the GTIN or GLN. gtinOrGLN=" + gtinOrgln + " and Exception=" + Ex.Message);
                        return null;
                    }
                }
            }
        ...
        public static async Task<bool> IsValidCompanyPrefixAsync(string companyPrefix)
        {
            ...
            gtin = gtin + GS1.CalculateGTIN14CheckSum(gtin);
