...
namespace gudusoft.gsqlparser.test
{
  ...
  set p.product_name = np.product_name";
        ...
        [TestMethod]
        public void TestIdentityClause()
        {
            TGSqlParser sqlparser = new TGSqlParser(EDbVendor.dbvoracle);
            sqlparser.sqltext = @"CREATE TABLE ""SYS"".""Test master""(
                                    id NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY(START with 11 INCREMENT by 22),
                                    ""id 2"" NUMBER,
                                    ""test description"" VARCHAR2(30) default('abc') NULL,
                                    col1 INTEGER NOT NULL,
                                    ""test date"" date DEFAULT(SYSDATE),
                                    constraint ""Test unique constraint"" UNIQUE(""test description""),
                                    constraint ""Test master PK"" primary key(id, ""id 2"")
                                    ); ";
            Assert.IsTrue(sqlparser.parse() == 0);
            TCreateTableSqlStatement createtable = (TCreateTableSqlStatement)sqlparser.sqlstatements.get(0);
            Assert.IsTrue(createtable.TargetTable.ToString().Equals(@"""SYS"".""Test master""", StringComparison.CurrentCultureIgnoreCase));
            TColumnDefinition columnDefinition = createtable.ColumnList.getColumn(0);
            Assert.IsTrue(columnDefinition.IdentityClause.ToString().Equals("GENERATED BY DEFAULT ON NULL AS IDENTITY(START with 11 INCREMENT by 22)", StringComparison.CurrentCultureIgnoreCase));
            Assert.IsTrue(columnDefinition.IdentityClause.IdentityOptions.Count == 2);
           // Console.WriteLine(columnDefinition.IdentityClause.IdentityOptions[0].ToString());
        }
    }
}
