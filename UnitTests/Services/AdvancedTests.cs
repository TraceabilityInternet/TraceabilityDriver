    ...
    public class AdvancedTests
    {
        ...
        public async Task Scenario1()
        {
            ...
            using (ITEInternalClient td02Client = TEClientFactory.InternalClient(td02URL, config02.APIKey))
            {
                ...
                string events = await td01Client.GetEventsAsync(account01.ID, tp01.ID, "urn:epc:class:lgtin:08600031303.00.METAL"); // EPC from first entry in Events.xml

                //// account #1 requests master data from account #2
                //string json = await td01Client.GetTradeItemAsync(account01.ID, tp01.ID, "urn:epc:idpat:sgtin:08600031303.00");
