...
namespace TraceabilityDriver.Services.GDST
{
    ...
    public class GDSTCapabilityTestService : IGDSTCapabilityTestService
    {
        ...
        private async Task IngestGeneratedDataAsync(GDSTCapabilityTestModel model)
        {
            ...
            // Capability-test data arrives through the traceback pipeline and is stored as traceback data,
            // so it is always served regardless of the configured deployment version.
            await _mongoDb.StoreTracebackEventsAsync(result.Document.Events);
            await _mongoDb.StoreTracebackMasterDataAsync(result.Document.MasterData);
