...
namespace TraceabilityDriver.Models.DB.Sql
{
    ...
    /// upserted by, plus the merged common event JSON so later sync runs can merge additional source
    /// rows into the same event. Traceback rows carry a null deployment version and no event key, and
    /// are never updated once stored.
    /// </remarks>
    public class EPCISEventSqlDocument
    {
        ...
        /// additional source rows into it. Null on traceback rows.
        /// </summary>
        public string? CommonEventJson { get; set; } = null;
