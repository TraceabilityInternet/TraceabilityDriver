...
namespace FDModels.StaticData
{
    public static class SpeciesList
    {
        ...
        private static List<Species> Load()
        {
            ...
            DSXML xmlSpeciesList = loader.ReadXML("FDModels.StaticData.Data.SpeciesList.xml");
            foreach (DSXML xmlSpecies in xmlSpeciesList)
            {
    ...
    public class Species : IEquatable<Species>, IComparable<Species>
    {
        ...
        public DSXML ToXML(DSXML xmlParent, string name = "Species")
        {
            DSXML xmlFM = xmlParent.AddChild(name);
            xmlFM.Attribute("Key", Key);
            return (xmlFM);
        }

        public static Species FromXML(DSXML xmlFM)
        {
